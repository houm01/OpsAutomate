...
MONGO_URL = 'mongodb://infra:EQKqKU2xdLF*G92@10.20.3.20:27017/infra-python'
...
FORMATTER_DICT = {
    ...
    6: logging.Formatter('%(name)s - %(asctime)-15s - %(filename)s - %(lineno)d - %(levelname)s: %(message)s',
                         "%Y-%m-%d %H:%M:%S"),
    # 7: logging.Formatter('%(asctime)s | %(name)s | %(levelname)s | "%(filename)s:%(lineno)d" | %(message)s', "%Y-%m-%d %H:%M:%S"),  # 一个只显示简短文件名和所处行数的日志模板
